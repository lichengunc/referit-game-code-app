...
var useFilteredWords = true;
var filteredWords = ['oclock', "o'clock", 'left', 'right', 'lefmost', 
                     'rightmost', 'front', '1st', '2nd',  
                     'center', 'top', 'bottom', 'middle', 'first', 'second', 'east', 'west'];
var filteredCharacters = "<>()|&^#@~-=+*%$^\/?\"{}'";
var gameRequirementsTxt = '(*) Remember you are <b>NOT ALLOWED</b> to include <b>ABSOLUTE LOCATION</b> information. Words like these are not allowed: left, right, center, middle, top, bottom, leftmost, rightmost. Work by players trying to bypass this restriction will not be accepted, so do not try leaving spaces between words to bypass this restriction. Also we are having problems with disconnections, please do not claim hits in advance, we have put a time restriction to try to avoid this.';
...
// Common words listing.
var filteredWordsTest = {};
for ( i = 0; i < filteredWords.length; i++ ) {
    filteredWordsTest[filteredWords[i]] = true;
}
...
var workerId;
var mTurkInstructions = 'Welcome to <span class="branding">ReferIt Game</span>! In moments you and another mturk player will both be shown the same picture. If you see an outlined item, then your goal is to describe the object as best as possible (ex: "The person wearing the hat", "The cat inside the box"). If you are the other player, then your goal is to select the object that the first player describes.  <br><br><span style="color:#cc0000;font-weight:bold">IMPORTANT:</span> You are not allowed to use absolute location in the description, certain words are in fact <span style="color:#cc0000;font-weight:bold">not</span> allowed by the game (<span style="color:#cc0000">left, right, center, bottom, top, leftmost, rightmost, first, second</span>). Also, remember to click "Accept HIT" before pressing Start.<br><br>You both earn points and swap roles if the second player selects the correct object. The faster you play, the more points you win. Please write the first phrase that comes to your mind--don\'t overspecify or underspecify; rather, write natural phrases that you would use if you were talking to a friend.\nYou will receive credit for correct games, and each HIT will consist of 5-10 games.';
...
function validateExpression(expression) {
    msg = '';
    if(useFilteredWords) {
        var wordArr = expression.match(/\w+/g); 
        for (i = 0; i < wordArr.length; i++) {
            word = wordArr[i].trim().toLowerCase();
            if (filteredWordsTest[word]) {
                msg = 'You are not allowed to use: ' + word;
                return msg;
            }
        } 
        for (i = 0; i < filteredCharacters.length; ++i) {
            if (expression.indexOf(filteredCharacters[i]) > -1) {
                msg = 'You are not allowed to use: ' + filteredCharacters[i];
                return msg;
            }
        }
    }
...
function enterText() {
	...
	if(expression == '' || expression == enterMessage)
        ...
        validateMsg = validateExpression(expression);
        if(validateMsg != '') {
            alert(validateMsg);
        } else {
            if(turn == 1) {
...
function turn1Visibility() {
    ...
    textMessage.html(gameRequirementsTxt);
}
