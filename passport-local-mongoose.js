...
var dbConn = mongoose.createConnection('mongodb://localhost/personalScoreDB');
...
module.exports = function(schema, options) {
    ...
    schema.statics.register = function(user, password, cb) {
        ...
        self.findByUsername(user.get(options.usernameField), function(err, existingUser) {
            ...
            var thisUserName = user.get(options.usernameField);

            //Create a new collection in the personal scores db for this user
            var personalScoreSchema = new mongoose.Schema({
                highScore: {type: Number},
                date: {type: Date}
            }, {collection: thisUserName});

            //personalScore here is the collection
            var personalScore = dbConn.model('RegModel', personalScoreSchema);

            var entry = new personalScore({
                highScore: 0,
                date: new Date()
            });
            ...
            personalScore.find({}, function(err, docs) {
                if(err) return console.error(err);
                console.log("docs from passport " + docs);
            });
